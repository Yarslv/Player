package com.yprodan.player.network.response
...
class Either<out T>(val value: Any?) {
    ...
    fun success(): T = (value as T)
    fun <T> failure(): Either<T> {
        if (isFailure) return Either(value)
        return Either(exceptionOrNull() as T)
    }
}
